...
function getData(page){
  ...
  $.ajax({
    type: 'GET',
    url: '/xxxknight/admin.php/Keyword/listKeyword',
    data: {'pageNum':page-1},
    dataType:'json',
    // beforeSend:function(){
    ...
    success:function(json){
      $("#keywordlist tbody").empty();
      if (!json.list) {
        var empty="<tr class='empty'><td colspan='5'>暂无数据</td></tr>";
        $("#keywordlist tbody").append(empty);
        $("#pagecount").html("");
        return;
      };
      ...
      $.each(list,function(index,array){ //遍历json数据列
        ...
        var value = $("<td></td>").append(array['value']);
        ...
        $("#del"+array['id']).click(function(){
            deleteKeyword(array['id']);
        });

        // $("body").on('click',"#update" + array['id'],function(){
        //     updateKeyword(array);
        // });
        // $("body").on('click',"#del" + array['id'],function(){
        //     deleteKeyword(array);
        // });
...
function deleteKeyword(id){
  if(confirm("是否删除该关键字")){
    $.get("/xxxknight/admin.php/Keyword/deleteKeyword",
      {'id':id},
...
$(function(){
    $("#btnUpdate").click(function(){
      ...
      $.post("/xxxknight/admin.php/Keyword/updateKeyword",
          {
              "id" : id,
              "name" : name,
              "value" : value,
          ...
          function(data,status){
            alert(data);
            window.location.href="/xxxknight/admin.php/Article/cloud";
        });
